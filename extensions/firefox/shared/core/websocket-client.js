...
const InspektWebSocketClient = (() => {
    // VM detection: Check if running inside the Browser VM (Linux + Chromium/Firefox)
    ...
    const BRIDGE_HTTP_PORT = isVMBrowser ? 8767 : 8765;
    const WS_URL = `ws://127.0.0.1:${WS_PORT}/ws`;
    const RECONNECT_DELAY = 3000;
    const KEEPALIVE_INTERVAL = 30000;

    if (isVMBrowser) {
        console.log('[Inspekt] VM environment detected - using isolated ports', { WS_PORT, BRIDGE_HTTP_PORT });
    }
    ...
    async function handleMessage(event) {
        try {
            ...
            if (message.type === 'execute') {
                ...
                // Send immediate acknowledgment - this lets the CLI know we received the request
                // and the browser is alive, BEFORE we do any async work
                ...
                const permModule = typeof InspektPermissions !== 'undefined' ? InspektPermissions : null;
                ...
                // Forward navigation request to background script
                // Uses callback mode because the content script will be destroyed during navigation
                // The background script will POST the result directly to the bridge server
                console.log('[Inspekt] NAVIGATE received:', message.url, 'requestId:', message.requestId, 'timeout:', message.timeout);
                const runtimeAPI = typeof chrome !== 'undefined' ? chrome.runtime : browser.runtime;
                try {
                    // Tell background to use callback mode - it will POST result to bridge
                    await runtimeAPI.sendMessage({
                        type: 'NAVIGATE',
                        url: message.url,
                        waitFor: message.waitFor || null,
                        timeout: message.timeout || 30,
                        requestId: message.requestId,
                        useCallback: true,
                        bridgePort: BRIDGE_HTTP_PORT
                    });
                    // Don't send response here - background will POST it directly to bridge
                    // The content script may be destroyed before navigation completes
                    console.log('[Inspekt] Navigation initiated, background will callback to bridge');
                } catch (err) {
                    console.error('[Inspekt] Error handling NAVIGATE:', err);
                    ws.send(JSON.stringify({
                        type: 'response',
                        requestId: message.requestId,
                        response: { ok: false, error: String(err) }
